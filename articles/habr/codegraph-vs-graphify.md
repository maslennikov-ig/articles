...
- **Явное разделение режимов** — `code/local graph` vs `full semantic` vs `blocked/deferred semantic`, с правилом «не запускать платный backend без явного разрешения». Это спасает, когда агент видит чьи-то API-ключи в `env` и решает, что ему «можно».
