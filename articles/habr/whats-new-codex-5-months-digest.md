...
Раньше, чтобы видеть, что делает дочерний агент, я делал так: оркестратор писал промпт в чат, я этот промпт мышкой копировал, открывал новую сессию Codex, вставлял, запускал, ждал, копировал результат обратно. Каждое параллельное действие — тройная ручная работа. Через хуки это нормально не делалось: я пробовал и ловил то, что хуки молча не срабатывают, то, что копируют не тот блок.

Сейчас я открываю Codex Desktop App, оркестратор спавнит субагентов, я кликаю в карточку любого и смотрю весь его трейс. Без копирования, без новых сессий. Это не «удобнее на 10%» — это другой класс работы.
...
Чтобы вы понимали масштаб, без оценок и комментариев — просто срез changelog:
...
Если коротко — у Codex есть два способа запуска вспомогательных агентов:

1. **Inline** — модель прямо в текущей сессии «изображает» субагента: думает за него, пишет от его лица, возвращает ответ. Наблюдаемости — ноль. Это просто часть основного трейса, и в нём всё перемешано.
2. **Spawn** — модель просит CLI запустить отдельного субагента как самостоятельный thread. У него своя карточка, свой токен-счётчик, свой статус. На него можно кликнуть и провалиться в его трейс целиком.

Когда subagents стали first-class workflow (март), spawn-вариант получил path-based адресацию (`/root/agent_a`, `/root/agent_a/agent_b1`). И вот это поменяло мне команду буквально.

До этого, повторюсь, ручная работа была такая: основная сессия пишет «было бы хорошо запустить субагента с таким промптом», я этот промпт копирую, открываю отдельный терминал, запускаю, дожидаюсь, копирую результат обратно. Параллельность была в кавычках, потому что я-то один. И через хуки автоматизировать это нормально не получалось — пробовал в разных вариантах, ловил либо отсутствие срабатывания, либо неправильный блок текста.
...
Сильнее всего меня радует, что hooks теперь можно навешивать на стадии оркестрации — например, после спавна субагента проверить, что у него установлен правильный write zone. Раньше это была мечта, теперь это работает.
...
**Лимиты у ChatGPT-плана сейчас БОЛЬШЕ, чем у Anthropic.** И сбрасываются регулярно. Я не помню за эти пять месяцев ни одной недели, когда бы я упёрся в Codex-лимит до конца сессии. У Anthropic сейчас тоже идёт акция с увеличенными лимитами, но они почему-то заканчиваются быстрее.
...
- **Оркестратор для Codex на базе Beads и Superpowers** — система из четырёх локальных skill, которая держит контракт оркестрации, чтобы Codex не «срезал углы». Это майская статья, и архив со всеми четырьмя скиллами я выкладывал в Telegram-канале. Если вы хотите поставить себе — пост с zip-вложением там до сих пор сверху закреплён.
...
Telegram-канал, где я скидываю заметки между статьями: [t.me/maslennikovigor](https://t.me/maslennikovigor). Архив скиллов оркестратора Codex — там же, отдельным закреплённым постом с zip-вложением. Прямой контакт: [@maslennikovig](https://t.me/maslennikovig).
