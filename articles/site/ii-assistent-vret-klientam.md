...
tldr: "ИИ-ассистент врёт клиентам не из-за плохой модели, а из-за пустоты вокруг неё. Цифры, артикулы и остатки у нас проверяет код – там провалов нет. Описания товара не проверял никто, и модели приписывали креслам характеристики, которых в каталоге нет. Когда мы наконец посчитали, что вообще лежит в каталоге, выяснилось худшее: вместимости стола как поля не существует, ассистент вынимал её регулярным выражением из текста описания, а описание в 89% случаев называет два разных числа. Часть галлюцинации жила в нашем собственном коде. После починки данных и раздельной оценки по трём осям картина перевернулась: модель с лучшим слогом оказалась худшей по фактам."
faq:
  ...
  - q: "Поможет ли вторая модель, которая проверяет первую?"
    a: "Частично, и доверять ей вслепую нельзя. Наш проверяющий ловил конкретную выдумку вроде «сетчатая спинка» и пропускал расплывчатую вроде «эргономичный дизайн с регулируемыми элементами» – притом что в контексте не было ни одного атрибута этого кресла. Хуже того, он штрафовал модель за честное «предположим, десять мест на стол – или разбить иначе?», хотя это единственно правильный ответ, когда данных нет. Проверка второй моделью обнаруживает часть проблем, но не устраняет их."
  ...
  - q: "Насколько это дорого?"
    a: "Сами ответы четырёх моделей на шестидесяти сценариях стоили 2,7 цента. А вот автоматическая проверка качества обошлась примерно в двадцать раз дороже, чем всё, что она проверяла: оценивающая модель читает каждый сценарий целиком плюс все ответы разом. Закладывайте её в бюджет отдельной строкой. Основные же затраты лежат не в токенах, а в работе над обвязкой: состояния инструментов, контракт утверждений, тестовый набор."
...
Заодно выяснилось, что мы ошиблись в собственной стоимости на два порядка. Раньше я называл цифру 0,59 доллара за раунд – она включала работу судьи. Ответы всех четырёх кандидатов на все шестьдесят сценариев стоили 2,7 цента. Судейство обошлось примерно в двадцать раз дороже, чем всё, что оно оценивало, и это не особенность нашего стенда: проверяющая модель читает каждый сценарий целиком плюс все ответы разом, а кандидат читает свой сценарий один раз. Если вы закладываете автоматическую проверку качества в бюджет, считайте её отдельной строкой – она крупнее самих ответов.
...
Прогоны я повторяю с каждой волной релизов, а результаты и разборы вроде «эта модель сломалась вот здесь» выкладываю в своём канале [@maslennikovigor](https://t.me/maslennikovigor). Там же появятся цифры по зарегулированности, когда мы наконец прогоним контрольный набор.
