...
Все цифры – из собственного прогона 4 августа 2026 года: семь замеров, 460 запросов, оплата по фактическому списанию, а не по прайсу. Тарифы живут неделями, поэтому у каждой суммы стоит дата.
...
Множители независимы: они перемножаются, а не заменяют друг друга. И ни один не требует менять модель.
...
Вывод для проекта простой: экономию считать по факту, а не по прайсу, и держать в коде путь отхода в стандартный класс.
...
Короткий список для разового аудита.

- Посмотреть полный список провайдеров своей модели, а не одну цену из общего справочника. Разница между первым и последним доходит до двадцати двух раз.
- Найти тарифный порог по длине контекста и проверить, не ходят ли ваши запросы за него.
- Вынести фоновые задачи в экономкласс с повторными попытками и эскалацией в стандартный класс по сроку.
- Разделить промпт на постоянную и переменную части, постоянную поставить в начало – это включает кэш.
- Считать по фактическому списанию, а не по прайс-листу. На прогоне 2 августа у GLM-5.2 фактическое списание вышло на 108% выше расчёта по прайсу: маршрутизатор увёл запрос к другому провайдеру.
- Записывать в журнал, кто именно обработал каждый запрос. Без этого нельзя объяснить, почему на прошлой неделе всё работало иначе.

Последние два пункта не требуют инженера вообще, и начинать стоит с них: пока расходы не измеряются, оптимизировать нечего.
