...
Запусти skill `cleanup-ai-noise` с параметром `platform=habr`. Скилл сохранит черновик в `.tmp/current/articles/`, прогонит агента `ai-text-checker` (34 паттерна + штампы проекта + платформо-специфика Хабра — доверие, непроверяемые цифры, AI-CEO-евангелизм, «каша из топора», «кодер vs инженер») и вернёт таблицу находок + спорные места (файл не изменяется) для ручного решения.
...
> Но после авторской вычитки модель к тексту не возвращается: ни стилистического
> прохода, ни «причешу мелочи».
...
**3. `cleanup-ai-noise` с `platform=habr` и `mode=report`.** **mode=report — обязательно.** Для Хабра `mode=edit` запрещён правилами площадки 2026.
...
Агент возвращает таблицу находок и не трогает файл. Правки вносит автор — после них переходи к ручным чек-листам ниже.
